...
// g++ -O3 -std=c++17 -mavx -mavx2 p1_modified.cpp -o p1 -pthread
...
struct word_tracker {
  alignas(64) uint64_t *word_count; //dynamically allocated array depending on no. of threads
  uint64_t total_lines_processed;
  uint64_t total_words_processed;
  pthread_mutex_t word_count_mutex;
} tracker __attribute__((aligned(64))); //actual culprit maybe

/*
1. uint64_t word_count[4] causes false sharing when multiple threads access consecutive elements
2. uint64_t total_lines_processed and total_words_processed cause false sharing when both of them get assigned same cache line and one thread accesses 1st variable and another thread accesses 2nd variable
*/

// Shared queue, to be read by producers
std::queue<std::string> shared_pq; //stores filepaths of N files to be read
...
int main(int argc, char *argv[]) {
  ...
  tracker.word_count = (uint64_t*)aligned_alloc(64, thread_count*sizeof(uint64_t)); //allocate a 64 byte aligned array to word_count
