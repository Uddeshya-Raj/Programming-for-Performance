...
#define hashT PhasedCuckooHashTable
...
void* thread_add(void* arg) {
  ...
  data->result[data->index] = data->ht->add(data->kv_pair->key, data->kv_pair->value);
  return nullptr;
}

void batch_insert(hashT& ht, KeyValue* kv_pairs, bool* result, int num_elements) {
  pthread_t threads[NUM_THREADS];
  ThreadData thread_data[NUM_THREADS];
  for (int i = 0; i < num_elements; ++i) {
    thread_data[i].ht = &ht;
    thread_data[i].kv_pair = &kv_pairs[i];
    thread_data[i].result = result;
    thread_data[i].index = i;

    if (pthread_create(&threads[i], nullptr, thread_add, (void*)&thread_data[i]) != 0) {
...
int main(int argc, char* argv[]) {
  ...
  bool insert_result[ADD] = {false};
  bool delete_result[REM] = {false};
  bool lookup_result[FIND] = {false};

  HRTimer start, end;
  for (int i = 0; i < runs; i++) {
    hashT ht = hashT(ADD);
    start = HR::now();
    batch_insert(ht, h_kvs_insert, insert_result, ADD);
    end = HR::now();
