...
__device__ void recursive_func(double (*output)[10], double *x, double *sums, int index, int* h_counter){
  if(index >= 10){
    bool good_sum = true;
    for(int k=0;k<10;k++){
  ...
  for(double xi=start; xi<end; xi+=step){
    ...
    if(bad_i) continue;
      ...
      new_x[index] = xi;
      recursive_func(output, new_x, temp, index+1, h_counter);
    }
  }
}

__global__ void wrapper_kernel(double (*output)[10], double *x, double *sums, int index, int *h_counter){
  ...
  if(!bad_values){
    ...
    new_x[1] = x1;
    new_x[2] = x2;
    recursive_func(output, new_x, temp, index+3, h_counter);
  }
}
...
int main() {
  ...
  double (*output)[10];
  cudaCheckError(cudaMallocManaged(&output, 12000 * 10 * sizeof(double)));

  int *h_counter;
  cudaCheckError(cudaMallocManaged(&h_counter, sizeof(int)));
  ...
  wrapper_kernel<<<blockpergird, 1>>>(output, d_x, d_sums, 0, h_counter);
  ...
  cudaEventElapsedTime(&kernel_time,start,end);
    ...
    int size = *h_counter;
  vector<vector<double>> data(size, vector<double>(10));
